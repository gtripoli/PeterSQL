...
class SettingsController:
    ...
    def _populate_shortcuts(self) -> None:
        ...
        shortcuts = self.settings.get_value("editor", "shortcuts", default={})
    ...
    def _load_advanced_settings(self) -> None:
        self.dialog.advanced_connection_timeout.SetValue(
            self.settings.get_value("runtime", "connection_timeout", default=60)
        )
        self.dialog.advanced_query_timeout.SetValue(
            self.settings.get_value("runtime", "query_timeout", default=60)
        )

        levels = [LogLevel.DEBUG, LogLevel.INFO, LogLevel.WARNING, LogLevel.ERROR]
        logging_level = self.settings.get_value("runtime", "logging_level", default="INFO")
        try:
    ...
    def _load_appearance_settings(self) -> None:
        current_theme = self.settings.get_value("ui", "appearance", "theme", default="")
        if current_theme:
            idx = self.dialog.theme.FindString(current_theme)
            if idx != wx.NOT_FOUND:
                self.dialog.theme.SetSelection(idx)

        appearance_mode = self.settings.get_value("ui", "appearance", "mode", default="auto")
    ...
    def _load_general_settings(self) -> None:
        ...
        language = self.settings.get_value("language", default="en_US")
    ...
    def _load_query_editor_settings(self) -> None:
        self.dialog.query_editor_statement_separator.SetValue(
            self.settings.get_value("editor", "statement_separator", default=";")
        )
        self.dialog.query_editor_trim_whitespace.SetValue(
            self.settings.get_value("editor", "trim_whitespace", default=False)
        )
        self.dialog.query_editor_execute_selected_only.SetValue(
            self.settings.get_value("editor", "execute_selected_only", default=False)
        )

        autocomplete = self.settings.get_value("editor", "autocomplete", "enabled", default=True)
        self.dialog.query_editor_autocomplete.SetValue(
            autocomplete if autocomplete is not None else True
        )

        autoformat = self.settings.get_value("editor", "autoformat", default=True)
    ...
    def _save_appearance_settings(self) -> None:
        ...
        self.settings.set_value("ui", "appearance", "mode", value=appearance_mode)
    ...
    def _save_query_editor_settings(self) -> None:
        self.settings.get_value("editor", default={})
        self.settings.get_value("editor", "autocomplete", default={})

        self.settings.set_value("editor", "statement_separator",
            value=self.dialog.query_editor_statement_separator.GetValue()
        )
        self.settings.set_value("editor", "trim_whitespace",
            value=self.dialog.query_editor_trim_whitespace.GetValue()
        )
        self.settings.set_value("editor", "execute_selected_only",
            value=self.dialog.query_editor_execute_selected_only.GetValue()
        )
        self.settings.set_value("editor", "autocomplete", "enabled",
            value=self.dialog.query_editor_autocomplete.GetValue()
        )
        self.settings.set_value("editor", "autoformat",
