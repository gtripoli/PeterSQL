...
class ConnectionsTreeController:
    ...
    def __init__(
        self,
        connections_tree_ctrl: wx.dataview.DataViewCtrl,
        ...
        self.connections_tree_ctrl.Bind(
            wx.dataview.EVT_DATAVIEW_ITEM_EDITING_DONE, self._on_item_editing_done
        )
        self.connections_tree_ctrl.Bind(
            wx.dataview.EVT_DATAVIEW_ITEM_START_EDITING, self._on_item_start_editing
        )
        ...
        self.connections_tree_ctrl.Bind(
            wx.dataview.EVT_DATAVIEW_ITEM_ACTIVATED, self._on_item_activated
        )
    ...
    def _on_selection_changed(self, event):
        item = event.GetItem()
        if not item.IsOk():
            item = self.connections_tree_ctrl.GetSelection()
    ...
    def edit_item(self, item: wx.dataview.DataViewItem):
        if not item.IsOk():
            return

        self._allow_next_edit = True
