...
class DotCompletionHandler:
    ...
    def get_completions(
    ...
    ) -> tuple[Optional[list[CompletionItem]], str]:
        ...
        try:
            ordered_columns = self._order_columns(table.columns)
            columns = [
                CompletionItem(
                    name=col.name,
    ...
    @staticmethod
    def _order_columns(columns: object) -> list[object]:
        try:
    ...
    def refresh(
        self, database: Optional[SQLDatabase], scope: Optional[object] = None
    ) -> None:
        self._database = database
        self._scope = scope
        self._build_table_index()
