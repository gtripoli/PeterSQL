...
class BaseCheckTests:
    ...
    def test_check_in_table_definition(self, session, database, create_users_table, datatype_class):
        ...
        age_column = session.context.build_empty_column(
            table,
            datatype_class.INTEGER if hasattr(datatype_class, 'INTEGER') else datatype_class.INT,
            name="age",
            is_nullable=True,
        )
        age_column.add()
    ...
    def test_check_create_and_drop(self, session, database, create_users_table, datatype_class):
